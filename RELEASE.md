...
2. Bump the version of the libraries running [npm-bump.go](./scripts/npm-bump/npm-bump.go) `go run ./scripts/npm-bump/npm-bump.go <new version>`. For example: `go run ./scripts/npm-bump/npm-bump.go 1.2.3`.
...
7. Run [release.go](./scripts/release/release.go) `go run ./scripts/release/release.go`.
