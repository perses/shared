...
// Matches: ${__data.fields["fieldName"]} or ${__data.fields['fieldName']} and with optional format ${__data.fields["fieldName"]:format}
const SINGLE_FIELD_REGEX = /\$\{__data\.fields\[(?:"|')([^"']+)(?:"|')\](?::([a-z]+))?\}/g;
...
export function replaceDataFields(
  ...
  // Replace __data.fields["fieldName"] patterns (supports single or double quotes)
  result = result.replaceAll(SINGLE_FIELD_REGEX, (_match, fieldName: string, format: string | undefined) => {
...
export function replaceDataFieldsBatch(
  ...
  // Replace aggregated access with format: ${__data.fields["fieldName"]:csv}
  result = result.replaceAll(SINGLE_FIELD_REGEX, (match, fieldName: string, format: string | undefined) => {
...
export function hasBatchPatterns(template: string): boolean {
  // Reset regex lastIndex
  INDEXED_FIELD_REGEX.lastIndex = 0;
  return INDEXED_FIELD_REGEX.test(template) || /\$\{__data\.fields\[["'][^"']+["']\]:[a-z]+\}/.test(template);
}
