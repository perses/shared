...
Components that depend on mutable TanStack Table instances, the live usage metrics context, or direct remote plugin
invocation explicitly opt out of compilation with `use no memo`. Oxlint still checks these functions, so incompatible
APIs and unsupported getters have narrow, documented lint exceptions alongside the compiler opt-outs.
